...
#define DEFAULT_ADVERT_OFF_TIME       100       //30000   //jcn 30 seconds
...
static uint8 pos_id=0x00;		//jcn
static uint8 pos_id2=0x00;		//jcn
...
static uint8  gapRole_AdvEnabled = TRUE;//FALSE;       //jcn TRUE;
...
/*        // Update the advertising data
        pos_id2 = 0x01;				//jcn
        VOID GAP_UpdateAdvertisingData( gapRole_TaskID,                 //jcn add
...
void GAPRole_Init( uint8 task_id )
{
  ...
  gapRole_profileRole = GAP_PROFILE_BROADCASTER | GAP_PROFILE_OBSERVER; //jcn
...
uint16 GAPRole_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & START_ADVERTISING_EVT )
  {
    if ( gapRole_AdvEnabled )
    {
      ...
      uint8  advChannelMap = GAP_ADVCHAN_ALL;
    ...
    case GAP_ADV_DATA_UPDATE_DONE_EVENT:
      {
        ...
        if ( pPkt->hdr.status == SUCCESS )
        {
          ...
          {
            // Start advertising
           if ( (gapRole_state == GAPROLE_STARTED)			//jcn add
              || (gapRole_state == GAPROLE_WAITING) )
           {
             pos_id      =       0x02;
             VOID osal_set_event( gapRole_TaskID, START_ADVERTISING_EVT );
