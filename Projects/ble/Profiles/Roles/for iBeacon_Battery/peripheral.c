...
#define BATTERY_LEVEL_UPD_EVT         0x0010  // jcn
...
static uint8  last_BatteryLevel;
  ...
  switch ( param )
  {
    ...
    case GAPROLE_ADVERT_DATA:
      if ( len <= B_MAX_ADV_LEN )
      {
        ...
        //------------ jcn ----------------
        if(last_BatteryLevel != gapRole_AdvertData[29])
        {
          VOID osal_set_event( gapRole_TaskID, BATTERY_LEVEL_UPD_EVT );

          last_BatteryLevel = gapRole_AdvertData[29];
        }
        //------------ jcn ----------------
...
//jcn  gapRole_ConnectionHandle = INVALID_CONNHANDLE;   // R?

  GAP_RegisterForHCIMsgs( gapRole_TaskID );

  // Initialize the Profile Advertising and Connection Parameters
//jcn  gapRole_profileRole = GAP_PROFILE_PERIPHERAL;
  gapRole_profileRole = GAP_PROFILE_BROADCASTER;

//jcn  VOID osal_memset( gapRole_IRK, 0, KEYLEN );      //R?
//jcn  VOID osal_memset( gapRole_SRK, 0, KEYLEN );      //R?
//jcn  gapRole_signCounter = 0;                         //R?

//jcn  gapRole_AdvEventType = GAP_ADTYPE_ADV_IND;
...
//jcn  VOID osal_snv_read( BLE_NVID_IRK, KEYLEN, gapRole_IRK );         //R?
//jcn  VOID osal_snv_read( BLE_NVID_CSRK, KEYLEN, gapRole_SRK );        //R?
//jcn  VOID osal_snv_read( BLE_NVID_SIGNCOUNTER, sizeof( uint32 ), &gapRole_signCounter );      //R?
}
...
uint16 GAPRole_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  //------------ jcn -----------
  if ( events & BATTERY_LEVEL_UPD_EVT )
  {
    bStatus_t stat;

    // Update the advertising data
    VOID GAP_UpdateAdvertisingData( gapRole_TaskID,
    ...
    return ( events ^ BATTERY_LEVEL_UPD_EVT );
  }
  //------------ jcn -----------
