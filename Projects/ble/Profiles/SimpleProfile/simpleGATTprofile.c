...
#ifdef HAL_UART_TRANS
      uint16 peripheral_connhandle = INVALID_CONNHANDLE;
...
//                                    INVALID_TASK_ID );
        GAPRole_GetParameter( GAPROLE_CONNHANDLE, &peripheral_connhandle );
        if( peripheral_connhandle != INVALID_CONNHANDLE )
        {
          noti.handle = simpleProfileAttrTbl[11].handle;        // handle of simpleProfileChar4
          noti.len = len;
      //    noti.value[0] = 0x0a;//(uint8)value[0];
      //    noti.value[1] = 0x0b;//value[1];
      //    noti.value[2] = 0x0c;//value[2];
      //    noti.value[3] = 0xdd;//value[2];
      //    noti.value[4] = 0xee;//value[2];
      //    noti.value[5] = 0xff;//value[2];
          memcpy(noti.value,value,len);

          GATT_Notification( peripheral_connhandle, &noti, FALSE );
        }
      }
#else
      if ( len == sizeof ( uint8 ) ) 
...
bStatus_t SimpleProfile_GetParameter( uint8 param, void *value )
{
  ...
  switch ( param )
  {
    ...
    case SIMPLEPROFILE_CHAR4:
#ifdef HAL_UART_TRANS
      VOID osal_memcpy( value, simpleProfileChar4, SIMPLEPROFILE_CHAR4_LEN );  //simpleProfileChar1 ÊÇÊý×éÃû
#else
